...
func GetHelperPropierdad(document string, tipo string) (dependencia map[string]interface{}, outputError map[string]interface{}) {
	defer func() {
	...
	if resp, err := request.GetJsonTest(urlTercero, &idTercero); err == nil && resp.StatusCode == 200 {
		if len(idTercero) == 0 || len(idTercero[0]) == 0 {
			...
			logs.Error(err)
			outputError = map[string]interface{}{
				"funcion": "GetHelperPropierdad - request.GetJsonTest(urlTercero, &idTercero)",
				"err":     err,
				"status":  "502",
			}
	...
	urlDependencia += "&query=TerceroPrincipalId%3A" + strconv.Itoa(tercero.Id)
	if resp, err := request.GetJsonTest(urlDependencia, &idDependencia); err == nil && resp.StatusCode == 200 {
		if len(idDependencia) == 0 || len(idDependencia[0]) == 0 {
			err := fmt.Errorf("No hay dependencias registradas con el id " + string(tercero.Id))
			logs.Error(err)
			outputError = map[string]interface{}{
				"funcion": "GetHelperPropierdad - request.GetJsonTest(urlDependencia, &idDependencia)",
				"err":     err,
				"status":  "502",
			}
			return nil, outputError
	...
	for _, idDep := range idDependencia[0] {
		...
		if resp, err := request.GetJsonTest(urlDependencias, &Dependencias); err == nil && resp.StatusCode == 200 {
			if len(Dependencias) == 0 {
				err := fmt.Errorf("No hay dependencias registradas con el id " + string(tercero.Id))
				logs.Error(err)
				outputError = map[string]interface{}{
					"funcion": "GetHelperPropierdad - request.GetJsonTest(urlDependencias, &Dependencias)",
					"err":     err,
					"status":  "502",
				}
				return nil, outputError

...
func GetJefeDependencia(dependenciaId int, query string) (jefeDependencia []map[string]interface{}, outputError map[string]interface{}) {
	...
	url := beego.AppConfig.String("OikosService") + "dependencia/" + fmt.Sprint(dependenciaId)
	if resp, err := request.GetJsonTest(url, &dependencia); err == nil && resp.StatusCode == 200 {
		urlTercero := beego.AppConfig.String("TercerosService") + "vinculacion?limit=-1"
		...
		if resp2, err2 := request.GetJsonTest(urlTercero, &terceros); err2 == nil && resp2.StatusCode == 200 {
			if len(terceros) > 0 {
				tercero = terceros[0]
				urlCargo := beego.AppConfig.String("ParametroService") + "parametro/" + fmt.Sprint(tercero.CargoId)
